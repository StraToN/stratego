...
// constructeur
Capitaine::Capitaine(bool col) : Unite(col,6){
    # ifdef DEBUG
    cout << "[" << this << "]Capitaine " << getColorValue(col) << " construit" << endl;
    # endif
}

// attaque la piece situee en (x,y) sur le plateau p
bool Capitaine::attack(Plateau* p,int x,int y){
    # ifdef DEBUG
...
char Capitaine::getCharID(){
    return C_CAPITAINE;
}

// retourne le nom
string Capitaine::getName(){
    return N_CAPITAINE;
}
